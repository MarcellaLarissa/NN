...
def read_input_vals(in_file):
    ...
    line = file.readline()## store as cities
    line = file.readline()	# iterator
    #points tracks the points as the key and the number of visitations as the value at that key
    points = []
    while len(line) > 1:
        line_parse = re.findall(r'[^,;\s]+', line)#  USE THIS TO PARSE
        points.append(int(line_parse[0]))## c array VETICES/cities [id, x, y]
        line = file.readline()
    file.close()## save as to numpy look up save and load numpy array every time to use it
    # if file does not exst, (array.npy)
        #read it, (read input vals)
    # if it exists load it (array.npy)## faster way to load stuff might need to number nmpy array
...
def read_output_vals(out_file): ## update with WRITING vals
